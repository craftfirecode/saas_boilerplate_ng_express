...
import {
  checkUsernameExists, updateUsername, changePassword, checkEmailExists,
  requestEmailChange, getUserByEmailChangeToken, confirmEmailChange,
  requestDeleteAccount, getUserByDeleteToken, deleteUserById,
  getUserById, getUserByEmail, requestPasswordReset,
  getUserByPasswordResetToken, resetPassword,
} from '../services/user.service.js';
import {
  usernameSchema, emailSchema, passwordResetSchema, emailChangeSchema,
} from '../utils/validators.js';
import {
  signEmailChangeToken, verifyEmailChangeToken,
  signDeleteAccountToken, verifyDeleteAccountToken,
  signPasswordResetToken, verifyPasswordResetToken,
} from '../utils/token.js';
import {
  sendEmailChangeMail, sendDeleteAccountMail, sendPasswordResetMail,
...
export async function updateProfile(req: Request, res: Response) {
  ...
  const parsed = usernameSchema.safeParse((req.body.username as string | undefined)?.trim());
...
export async function updatePassword(req: Request, res: Response) {
  ...
  if (!currentPassword) return res.status(400).json({ error: 'Alle Felder sind erforderlich.' });
  ...
  if (result.error === 'wrong_password') return res.status(422).json({ error: 'Das aktuelle Passwort ist falsch.' });
  if (result.error === 'not_found') return res.status(404).json({ error: 'Benutzer nicht gefunden.' });
...
export async function requestEmailUpdate(req: Request, res: Response) {
  ...
  const parsed = emailChangeSchema.safeParse({
    email: (req.body.email as string | undefined)?.trim().toLowerCase(),
  });
  if (!parsed.success) return res.status(400).json({ error: parsed.error.issues[0]?.message });
...
export async function confirmEmailUpdate(req: Request, res: Response) {
  ...
  if (taken) return res.status(409).json({ error: 'Diese E-Mail-Adresse wird bereits von einem anderen Account verwendet.' });
...
export async function requestAccountDeletion(req: Request, res: Response) {
  ...
  const confirmedEmail = (req.body.email as string | undefined)?.trim().toLowerCase();
...
export async function requestPasswordResetHandler(req: Request, res: Response) {
  const parsed = emailSchema.safeParse((req.body.email as string | undefined)?.trim().toLowerCase());
  if (!parsed.success) return res.status(400).json({ error: parsed.error.issues[0]?.message });
