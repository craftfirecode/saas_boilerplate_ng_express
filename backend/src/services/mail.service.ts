...
async function createTransporter() {
  ...
  try {
    ...
    console.warn(`[Mail] DNS-Lookup für "${host}" fehlgeschlagen, verwende Hostname direkt:`, (dnsErr as Error).message);
  }
...
export async function sendMail(options: SendMailOptions, attempt = 1) {
...
export async function sendEmailChangeMail(to: string, username: string, newEmail: string, token: string) {
  const appUrl = process.env['APP_URL'] ?? 'http://localhost:4200';
...
export async function sendDeleteAccountMail(to: string, username: string, token: string) {
  const appUrl = process.env['APP_URL'] ?? 'http://localhost:4200';
...
export async function sendPasswordResetMail(to: string, username: string, token: string) {
  const appUrl = process.env['APP_URL'] ?? 'http://localhost:4200';
