...
const router = Router();
router.get('/', verifyAccess, async (_req, res) => {
    try {
...
router.get('/:id', verifyAccess, async (req, res) => {
    ...
    try {
        const folder = await prisma.folder.findFirst({ where: { id }, include: { todos: true } });
        if (!folder)
            return res.status(404).json({ error: 'Folder nicht gefunden' });
        res.json(folder);
    }
    catch {
        res.status(500).json({ error: 'Fehler beim Laden' });
...
router.post('/', verifyAccess, async (req, res) => {
    try {
        const folder = await prisma.folder.create({
            data: { name: req.body.name, userId: Number(req.user?.sub) },
        });
        res.json(folder);
    }
    catch {
...
router.delete('/:id', verifyAccess, async (req, res) => {
    ...
    try {
        ...
        if (!folder)
            return res.status(404).json({ error: 'Folder nicht gefunden' });
        await prisma.$transaction([prisma.folder.delete({ where: { id } })]);
